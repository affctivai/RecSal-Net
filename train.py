...
parser.add_argument('--no_epochs', default=500, type=int)
...
parser.add_argument('--train_path_data', default="./data/DHF1K/data/train", type=str)
parser.add_argument('--val_path_data', default="./data/DHF1K/data/val", type=str)
...
model = Saliencymodel(pretrain=args.load_path)
    ...
    if epoch % 3 == 0:
        with torch.no_grad():
            ...
            if best_loss < cc_loss:
                best_loss = cc_loss
                if torch.cuda.device_count() > 1:
                    torch.save(model.module.state_dict(), args.model_val_path + 'RecSalNet_{}.pth'.format(epoch))
                else:
                    torch.save(model.state_dict(), args.model_val_path + 'RecSalNet_{}.pth'.format(epoch))
writer.close()
